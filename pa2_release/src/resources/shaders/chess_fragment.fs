...
{
    vec4 baseColour; 
    int x = int(ceil(outTexCoord.x*16));//quantizing texture coordinates
    int y = int(ceil(outTexCoord.y*16));
    

    if (mod(x+y,2)==1)//generating checkerboard-like texture
    {
    	baseColour = vec4(1.0,1.0,1.0,1.0);
    }
    else
    {
    	baseColour = vec4(0.0,0.0,0.0,1.0);
    }
	...
	if ( material.useColour == 1 )
    {
    	...
    	baseColour=baseColour+vec4(material.colour,0);
    	baseColour=baseColour-vec4(material.colour,0);
    }
